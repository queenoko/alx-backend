#!/usr/bin/env python3
"""This script is the Least Frequently Used caching module.
"""
...
class LFUCache(BaseCaching):
    ...
    def __reorder_items(self, mru_key):
        """The mode Reorders is  the items in this cache based on the most
        recently used item.
        """
        max_positions = []
        mru_freq = 0
        mru_pos = 0
        ins_pos = 0
        for z, key_freq in enumerate(self.keys_freq):
            if key_freq[0] == mru_key:
                mru_freq = key_freq[1] + 1
                mru_pos = z
                break
            elif len(max_positions) == 0:
                max_positions.append(z)
